			   ...
			   uint8_t *rx_buf, uint32_t rx_len, uint16_t delay_ms)
{
	...
	k_usleep(500); // Wait for Hub to wake up
...
static int max32664c_afe_read_reg(const struct device *dev, uint8_t reg, uint8_t *val)
{
...
{
    uint8_t r11 = 0, r12 = 0, r20 = 0, r21 = 0, r22 = 0, r23 = 0, r24 = 0, r25 = 0;

    max32664c_afe_read_reg(dev, 0x11, &r11);  // PPG config 1
    max32664c_afe_read_reg(dev, 0x12, &r12);  // PPG config 2
    max32664c_afe_read_reg(dev, 0x20, &r20);  // LED sequence 1
    max32664c_afe_read_reg(dev, 0x21, &r21);  // LED sequence 2
    max32664c_afe_read_reg(dev, 0x22, &r22);  // LED sequence 3
    max32664c_afe_read_reg(dev, 0x23, &r23);  // LED1 current
    max32664c_afe_read_reg(dev, 0x24, &r24);  // LED2 current
    max32664c_afe_read_reg(dev, 0x25, &r25);  // LED3 current

    uint8_t ppg_sr  = (r12 >> 3) & 0x1F;
    uint8_t smp_ave = r12 & 0x07;

    printk("AFE REGS: 0x11=0x%02X 0x12=0x%02X 0x20=0x%02X 0x21=0x%02X 0x22=0x%02X 0x23=0x%02X 0x24=0x%02X 0x25=0x%02X\n",
           r11, r12, r20, r21, r22, r23, r24, r25);
    printk("AFE DECODE: PPG_SR=0x%02X SMP_AVE=0x%02X\n", ppg_sr, smp_ave);
}
...
int max32664c_set_mode_raw(const struct device *dev)
{
	...
	printk("-> RAW Mode Check 1: Stopping current algo...\n");
	...
	printk("-> RAW Mode Check 2.5: Setting FIFO threshold...\n");
	tx[0] = 0x10;
	tx[1] = 0x01;
	tx[2] = 0x01; /* Trigger DATA_RDY when 1 sample is in the FIFO */
	...
	/* Enable the AFE */
	printk("-> RAW Mode Check 3: Enabling AFE...\n");
	tx[0] = 0x44;
	tx[1] = 0x00;
	tx[2] = 0x01;
	tx[3] = 0x00;
	...
	printk("-> Setting AFE ADC Range ...\n");
	tx[0] = 0x40; // Write command
	tx[1] = 0x00;
	tx[2] = 0x11; // Register address
	tx[3] = 0x17;
	if (max32664c_i2c_transmit(dev, tx, 4, &rx, 1, MAX32664C_DEFAULT_CMD_DELAY)) {
		return -EINVAL;
	}
	...
	printk("-> RAW Mode Check 7: Dumping AFE config after setup...\n");


	/* --- INJECT THIS NEW BLOCK --- */
	printk("-> RAW Mode Check 6.5: Programming AFE LED Sequence...\n");
	...
	// tx[0] = 0x40; tx[1] = 0x00; tx[2] = 0x22; tx[3] = 0x00;
	// max32664c_i2c_transmit(dev, tx, 4, &rx, 1, 2);

	max32664c_dump_afe_config(dev);
    /* ----------------------------- */
 ...
 *  @return             0 when successful
 */
int max32664c_set_mode_algo(const struct device *dev, enum max32664c_device_mode device_mode,
...
static int max32664c_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	case MAX32664C_OP_MODE_RAW:
        /* Only return 0 (Success) if we actually got a message from the queue */
		if (k_msgq_get(&data->raw_report_queue, &data->raw, K_NO_WAIT) == 0) {
            return 0;
        }
        return -ENODATA; /* Tell main.c the queue is empty! */
	}
}
			      ...
			      enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_MAX32664C_DAC_OFFSET: {
        // Renamed local variable to 'reg_val' to avoid conflict with '*val'
        uint8_t reg_val = 0x80 | (val->val1 & 0x3F);

        uint8_t tx[] = { 0x40, 0x00, 0x2C, reg_val };
        return max32664c_i2c_transmit(dev, tx, 4, &rx, 1, 10);
    }

    case SENSOR_ATTR_MAX32664C_INTEGRATION_TIME: {
        // Renamed local variable to 'reg_val' to avoid conflict with '*val'
        uint8_t reg_val = (val->val1 & 0x03) | 0x08;

        uint8_t tx[] = { 0x40, 0x00, 0x11, reg_val };
        return max32664c_i2c_transmit(dev, tx, 4, &rx, 1, 10);
    }
	case SENSOR_ATTR_MAX32664C_PPG_CONFIG_1: {
        // Register 0x11: PPG Configuration 1
        // This controls both ADC Range (Gain) and Integration Time
        uint8_t reg_val = val->val1 & 0xFF;
